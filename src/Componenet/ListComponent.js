...
function ListComponent(props){ 
  ...
  function dateFormat(listdate){ 
    ...
    function Tbody(props){
        return  ( <tbody>
        {filterData(props.reminderList,props.action).map(listdata=>(
           <tr key={listdata.id}>
        ...
        <div className="row mt-4 pt-4">
            <div className="col-md-6">
                <h3>Past Reminder</h3>
                {
                    <table className="table">
                       <Thead />
                       <Tbody reminderList={props.reminderList} action="past"/ >
                    </table>
                }    
            ...
            <div className="col-md-6">
                <h3>Future Reminder</h3>
                {
                    <table className="table">
                      <Thead />
                      <Tbody reminderList={props.reminderList} action="future"/ >
                    </table>
                }    
            </div>
        </div>
